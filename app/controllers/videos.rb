...
#  end
  ...
  def upload
    ...
    begin
      ...
      @video.filename = @video.key.to_s + rand(99).to_s + File.extname(params[:file][:filename]) # This is for Green Thing #   # I'm an idiot, key == id
      FileUtils.mv params[:file][:tempfile].path, @video.tmp_filepath 
      @video.original_filename = params[:file][:filename] # Split out any directory path Windows adds in
      @video.process
